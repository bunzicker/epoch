...
CONTAINS
  ...
  SUBROUTINE output_routines(step, force_write)   ! step = step index
    ...
    DO iprefix = 1,SIZE(file_prefixes)
      ...
      IF (cpml_boundaries) THEN
        ...
        CALL write_field(c_dump_cpml_psi_exz, code, 'cpml_psi_exz', &
            'CPML/Ex_z', 'A/m^2', c_stagger_cell_centre, cpml_psi_exz)
        CALL write_field(c_dump_cpml_psi_eyz, code, 'cpml_psi_eyz', &
            'CPML/Ey_z', 'A/m^2', c_stagger_cell_centre, cpml_psi_eyz)
        CALL write_field(c_dump_cpml_psi_bxz, code, 'cpml_psi_bxz', &
            'CPML/Bx_z', 'A/m^2', c_stagger_cell_centre, cpml_psi_bxz)
        CALL write_field(c_dump_cpml_psi_byz, code, 'cpml_psi_byz', &
        ...
        IF (.NOT. sub%skip) THEN
          ...
          IF (.NOT. use_offset_grid) THEN
            CALL sdf_write_srl_plain_mesh(sdf_handle, TRIM(temp_block_id), &
                ...
                yb_global(ranges(1,2):ranges(2,2)), &
                zb_global(ranges(1,3):ranges(2,3)), convert)
          ELSE
            CALL sdf_write_srl_plain_mesh(sdf_handle, TRIM(temp_block_id), &
                TRIM(temp_name), xb_offset_global(ranges(1,1):ranges(2,1)), &
                yb_offset_global(ranges(1,2):ranges(2,2)), &
                zb_offset_global(ranges(1,3):ranges(2,3)), convert)
          END IF
        ELSE
          DO i = 1, io - 1
          ...
          IF (.NOT. use_offset_grid) THEN
            ...
            CALL sdf_write_srl_plain_mesh(sdf_handle, TRIM(temp_block_id), &
                TRIM(temp_name), x_reduced, y_reduced, z_reduced, convert)
          ELSE
          ...
          DEALLOCATE(x_reduced, y_reduced, z_reduced)
        END IF
      ...
      IF (IAND(iomask(c_dump_virtual_det_times), code) /= 0) THEN
        CALL sdf_write_srl(sdf_handle, 'detector_times', &
          'Virtual_Detector_Times', det_times, 0)
      END IF
      ...
      ELSE
        ...
        DO k = 1, rnz
          ...
          DO j = 1, rny
            ii = sub%n_start(1) + 1
            DO i = 1, rnx
              reduced(i,j,k) = array(ii,jj,kk)
              ii = ii + sub%skip_dir(1)
            END DO
            jj = jj + sub%skip_dir(2)
          END DO
          kk = kk + sub%skip_dir(3)
        END DO
  ...
  SUBROUTINE write_nspecies_field(id, code, block_id, name, units, stagger, &
    ...
    CHARACTER(LEN=*), INTENT(IN) :: block_id, name, units
    INTEGER, INTENT(IN) :: stagger
    REAL(num), DIMENSION(1-ng:,1-ng:,1-ng:), INTENT(OUT) :: array
    ...
    IF (dump_species .AND. dump_skipped) THEN
      ...
      IF (.NOT.ALLOCATED(reduced)) ALLOCATE(reduced(rnx,rny,rnz))
